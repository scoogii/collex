...
const TradesIncoming = ({
  ...
  const handleAcceptTrade = async (id) => {
    const response = await fetch(
      `http://localhost:8080/trading/complete_trade?trade_id=${id}`,
      {
        method: "POST",
        headers: {
    ...
    if (data.error) {
      ...
      getIncomingTrades();
      getHistoricalTrades();
      setSuccessMessage("Trade successfully accepted!");
      setSuccessOpen(true);
    }
  };

  const handleDeclineTrade = async (id) => {
    const response = await fetch(
      `http://localhost:8080/trading/decline_trade?trade_id=${id}`,
      {
        method: "POST",
        headers: {
          "Content-type": "application/json",
    ...
    if (data.error) {
      ...
      getIncomingTrades();
      getHistoricalTrades();
      setSuccessMessage("Trade successfully declined!");
      setSuccessOpen(true);
    }
  };
    ...
    <>
      ...
      <Paper elevation={0} sx={tradeContainerStyle}>
        {incomingTrades.length === 0 && (
          ...
          >
            <Typography>No incoming trades.</Typography>
          </Box>
        )}
            ...
            {isProfile && (
              <Box sx={{ width: "15%", marginLeft: "3%" }}>
                ...
                >
                  ...
                  >
                    {localStorage.getItem("username") === trade["target"] && (
                      <>
                        <Button
